  ...
  for (const [key, value] of Object.entries(attrs)) {
    ...
    if (key === 'style') {
      ...
      for (const declaration of value.split(';')) {
        ...
        const camel = property.startsWith('--') ? property : property.replace(/-([a-z])/g, (_, c: string) => c.toUpperCase())
        style[camel] = declaration.slice(colon + 1).replace(/!important/g, '').trim()
      }
      result.style = style as CSSProperties
...
function MediaCarousel({ node }: { node: Exclude<ContentNode, string> }) {
...
export function SourceSection({ node }: { node: Exclude<ContentNode, string> }) {
  return createElement(node.tag, reactProps(node.attrs), children(node.children))
}

export default function ContentRenderer({ node }: { node: ContentNode }) {
